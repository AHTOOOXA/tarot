...
function isCurrentDay(day: number | string): boolean {
  ...
  return (
    day === currentDate.getDate() && month.value === currentDate.getMonth() && year.value === currentDate.getFullYear()
  );
}
...
function isDisabledDate(day: number | string): boolean {
  ...
  if (props.minDate !== undefined) {
    ...
    if (
      year.value === props.minDate.getFullYear() &&
      month.value === props.minDate.getMonth() &&
      day < props.minDate.getDate()
    ) {
      return true;
    }
  }
  ...
  if (props.maxDate !== undefined) {
    ...
    if (
      year.value === props.maxDate.getFullYear() &&
      month.value === props.maxDate.getMonth() &&
      day > props.maxDate.getDate()
    ) {
      return true;
    }
  }
...
function withDaysChange(changer: () => void, direction: 'left' | 'right'): void {
  ...
  daysRef.value?.addEventListener(
    'transitionend',
    () => {
      changer();
      void nextTick(() => {
...
<template>
  <div class="date-picker">
    <div class="header">
      <div class="month-and-year">{{ monthName }} {{ year }}</div>
...
.day {
  ...
  &--selected {
    ...
    &::before {
      ...
      z-index: 1;
      opacity: 0.24;
      animation: circle-enter 200ms cubic-bezier(0.4, -0.31, 0, 1.23);
